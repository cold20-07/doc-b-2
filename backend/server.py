...
# Add compatibility routes for /undefined/api (React app has undefined REACT_APP_BACKEND_URL)
@app.get("/undefined/api/available-slots")
async def undefined_get_available_slots(date: str):
    return await get_available_slots(date)

@app.post("/undefined/api/appointments")
async def undefined_create_appointment(appointment: AppointmentCreate):
    return await create_appointment(appointment)

@app.post("/undefined/api/create-payment-order")
async def undefined_create_payment_order(order: PaymentOrder):
    return await create_payment_order(order)

@app.post("/undefined/api/verify-payment")
async def undefined_verify_payment(verification: PaymentVerification):
    return await verify_payment(verification)
...
async def serve_spa(full_path: str):
    ...
    if full_path.startswith("api/") or full_path.startswith("undefined/"):
